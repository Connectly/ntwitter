...
function Twitter(options) {
  ...
  this.debug = this.options.debug_log_function || debuglog('ntwitter');
 ...
 * GET
 */
Twitter.prototype.get = function(url, params, callback, useJSONBigInt) {
  ...
  if (url.charAt(0) == '/')
	...
	this.debug('GET: [%s]', url + '?' + querystring.stringify(params));
...
Twitter.prototype.getStatusesOEmbed = function(params, callback) {
...
Twitter.prototype.getListOwnerships = function(id, params, callback) {
  if (typeof params === 'function') {
    callback = params;
    params = null;
  }
...
Twitter.prototype.getListMembers = function(id, params, callback) {
  if (typeof params === 'function') {
    callback = params;
    params = null;
  }
...
Twitter.prototype.getFriendsIds = function(id, params, callback) {
  ...
  var params = utils.merge(params || {}, { key: 'ids', stringify_ids: 'true' });
  if (typeof id === 'string')
    params.screen_name = id;
  ...
  this._getUsingCursor(url, params, callback, true);
  return this;
}
...
Twitter.prototype._getUsingCursor = function(url, params, callback, useJSONBigInt) {
  var self = this,
    params = params || {},
    key = params.key || null,
    max_pages = params.ntwitter_max_pages || -1,
    result = [];
  ...
  // optional limit on how many pages we retrieve
  if(params.ntwitter_max_pages) {
    delete params.ntwitter_max_pages;
  }

  // kick off the first request, using cursor -1
  params = utils.merge(params, {cursor:-1});
  this.get(url, params, fetch, useJSONBigInt);
  ...
  function fetch(err, data) {
    ...
    if (data.next_cursor_str === '0' || max_pages === 0) {
      callback(null, result);
    } else {
      params.cursor = data.next_cursor_str;
      self.get(url, params, fetch, useJSONBigInt);
    }
  }
